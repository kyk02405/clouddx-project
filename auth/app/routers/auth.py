...
async def google_login(
    ...
    google_auth_url = (
        ...
        f"&redirect_uri={_oauth_callback_uri('google', request=request)}"
...
async def google_callback(code: str, state: str, request: Request):
    ...
    # 4. JWT  + ???
    response = await _oauth_issue_token_and_redirect(user_id, email, request=request)
...
async def kakao_login(
    ...
    kakao_auth_url = (
        ...
        f"&redirect_uri={_oauth_callback_uri('kakao', request=request)}"
        f"&response_type=code"
        f"&prompt=select_account"
        f"&state={oauth_state}"
    )
...
async def naver_login(
    ...
    naver_auth_url = (
        ...
        f"&redirect_uri={_oauth_callback_uri('naver', request=request)}"
        f"&state={oauth_state}"
