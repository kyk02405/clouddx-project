...
const OCR_API_URL = 'http://localhost:8002'; // OCR 전용 서버
    ...
    const handleNext = async () => {
        if (selectedFile) {
            ...
            try {
                ...
                // Draft 결과 조회
                const draftResponse = await fetch(`${OCR_API_URL}/import/draft/${uploadData.import_id}`);
                ...
                if (draftData?.items) {
                    const pendingData = draftData.items.map((item: any) => ({
                        ...
                        symbol: item.symbol,
                        name: item.symbol,
                        type: item.asset_type === "stock" ? "stock" : (item.asset_type === "crypto" ? "crypto" : "currency"),
                            ...
                            <div className="space-y-6 md:space-y-8 h-full flex flex-col">
                                <div className="space-y-3 md:space-y-2">
                                    ...
                                    <p className="text-sm md:text-lg text-zinc-600 dark:text-zinc-400 font-medium leading-relaxed">
                                        증권사 앱의 잔고 화면이나 엑셀 표를 캡처하여 업로드해 주세요.<br className="hidden md:block" />
                                        AI가 종목명, 수량, 평단가를 자동으로 인식하여 통합 등록 폼으로 연결합니다.
                                    </p>
                                </div>
                                        ...
                                        {isProcessing && (
                                            <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/80 dark:bg-zinc-950/80 backdrop-blur-sm">
                                                <div className="flex flex-col items-center">
                                                    ...
                                                    <h3 className="text-2xl font-black text-zinc-900 dark:text-white mb-2">AI 엔진 분석 중...</h3>
                                                    <p className="text-zinc-500 dark:text-zinc-400 font-medium">종목명과 평단가를 정밀 분석하고 있습니다.</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <div className="p-4 md:p-8 rounded-2xl md:rounded-3xl bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800">
                                    <div className="flex items-start gap-4 mb-8">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs md:text-sm text-zinc-500 dark:text-zinc-400">필수 항목들이 포함된 화면을 촬영해 주세요.</p>
                                        </div>
                                    </div>
