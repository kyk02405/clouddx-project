...
function AiAvatar({ size = 'h-8 w-8', iconSize = 'h-4 w-4' }: { size?: string; iconSize?: string }) {
    return (
        <div className={`flex ${size} flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-purple-500 to-fuchsia-500 text-white shadow-lg`}>
            ...
            <div className="max-w-[85%] flex-1">
                <div className="rounded-2xl rounded-tl-sm bg-zinc-100 px-4 py-3 shadow-sm dark:bg-zinc-800/50">
                    {content ? (
                        ...
                        isStreaming && <span className="text-zinc-400">답변을 작성하는 중입니다...</span>
                    )}
                ...
                {sources && sources.length > 0 && !isStreaming && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                        ...
                        {sources.map((source, idx) => (
                            ...
                            >
                                {source.type === 'price' && '시세 '}
                                {source.type === 'news' && '뉴스 '}
                                {source.type === 'portfolio' && '자산 '}
                                {source.title}
                            </span>
                        ))}
                    </div>
    ...
    if (messages.length === 0) {
        ...
        return (
            <div className="flex flex-1 flex-col items-center justify-center p-8 text-center">
                <div className="mb-6">
                    <AiAvatar size="h-16 w-16" iconSize="h-8 w-8" />
                </div>
