...
Phase 2: CI/CD 파이프라인 구축
  → gitlab.com (SaaS) + SonarQube (Monitoring VM) + GitLab Container Registry + ArgoCD (K8s 내 GitLab Runner)
...
# Calico Typha (calico-node → typha 통신, 필수!)
sudo ufw allow from 192.168.56.0/24 to any port 5473 proto tcp
...
# GitLab Container Registry (registry.gitlab.com) + gitlab.com 접근
# HTTPS(443)이므로 insecure registry 설정 불필요
sudo ufw allow out to any port 443 proto tcp

# SonarQube (Monitoring VM) 접근
sudo ufw allow from 192.168.56.0/24 to 192.168.56.30 port 9000 proto tcp
```
...
> **중요**: CI/CD는 **gitlab.com (SaaS)** + **SonarQube (Monitoring VM)** + **GitLab Container Registry**를 사용합니다.
> 자체 CI/CD VM은 사용하지 않습니다. GitLab Runner만 K8s 클러스터 내에 Helm chart로 설치합니다.
> SonarQube는 Monitoring VM(192.168.56.30)에 Docker Compose로 설치합니다.
> 기존 Docker Compose 3-tier VM(node1~3)은 K8s 마이그레이션 후 폐기 대상이므로 절대 사용하지 않습니다.

### 2-0. 사전 준비 (gitlab.com + SonarQube + Jira + Slack 설정)
...
**2-0-2. SonarQube 설치 (Monitoring VM: 192.168.56.30):**

> SonarQube는 Phase 2에서 필요하므로 Monitoring VM에 Docker를 먼저 설치하고 SonarQube를 띄웁니다.
> Phase 3에서 LGTM 스택을 추가할 때 같은 VM의 별도 Compose로 운영합니다.

```bash
# Monitoring VM에 SSH 접속
ssh clouddx@192.168.56.30

# Docker 설치 (아직 안 되어 있는 경우)
...
sudo chown clouddx:clouddx /opt/sonarqube

# sysctl 설정 (Elasticsearch 요구사항)
...
**SonarQube 초기 설정 (웹 브라우저에서):**
```
1. http://192.168.56.30:9000 접속
   - 초기 ID: admin / PW: admin → 비밀번호 변경 (예: tutum2026!)

2. 프로젝트 2개 생성:
   → Administration → Projects → Create Project
   - Project key: tutum-backend (Python)
   - Project key: tutum-frontend (TypeScript/JavaScript)

3. 토큰 생성:
   → My Account → Security → Generate Tokens
   - Name: gitlab-ci
   - Type: Global Analysis Token
   → 생성된 토큰을 GitLab CI Variables에 SONAR_TOKEN으로 등록

4. Quality Gates 설정 (Quality Gates → Create):
   - Name: tutum-gate
   - Conditions:
...
**2-0-3. Jira 연동 (gitlab.com 웹 UI에서):**
```
1. Jira Cloud 무료 가입: https://www.atlassian.com/software/jira/free
   - 프로젝트 생성: "TUTUM" (프로젝트 키: TUTUM)
   - 이슈 유형: Story, Bug, Task, Epic

2. GitLab ↔ Jira 연동:
   → gitlab.com → tutum-project → Settings → Integrations → Jira
   - Web URL: https://<your-domain>.atlassian.net
   - Username: Jira 이메일
   - Password/Token: Jira API Token (https://id.atlassian.com/manage-profile/security/api-tokens)
   - Project keys: TUTUM
   ✅ Enable Jira issue transitions (커밋 메시지로 이슈 자동 전환)

3. Smart Commit 사용법:
   git commit -m "TUTUM-123 fix: 로그인 버그 수정"
   # → Jira 이슈 TUTUM-123에 자동 코멘트 추가
   git commit -m "TUTUM-123 #close fix: 완료"
   # → 이슈 자동 종료 (Done 전환)
```

**2-0-4. Slack 연동 (gitlab.com + Grafana):**
```
1. Slack 워크스페이스 생성 (또는 기존 사용)
   - 채널 생성: #tutum-cicd (CI/CD 알림), #tutum-alerts (모니터링 알림)
...
3. GitLab ↔ Slack 연동:
   → gitlab.com → tutum-project → Settings → Integrations → Slack notifications
   - Webhook: (위에서 복사한 URL)
   - ✅ Push, Merge request, Pipeline, Deployment
   - Channel: #tutum-cicd

4. Grafana ↔ Slack 연동 (Phase 3에서 설정):
   → Grafana → Alerting → Contact points → New contact point
   - Name: slack-alerts
   - Type: Slack
   - Webhook URL: (별도 #tutum-alerts 채널 Webhook URL)
```
...
```
SONAR_TOKEN        = (SonarQube에서 발급한 Global Analysis Token)
...
SLACK_WEBHOOK_URL  = (Slack Incoming Webhook URL - #tutum-cicd 채널)
JIRA_PROJECT_KEY   = TUTUM
```
...
```yaml
# .gitlab-ci.yml (최종본 - gitlab.com + SonarQube + GitLab Container Registry + Slack/Jira)
stages:
  - lint
  - test
  - scan
  ...
  script:
    ...
    - |
      curl -X POST "$SLACK_WEBHOOK_URL" \
        ...
        -d "{
          \"text\": \":white_check_mark: *Pipeline 성공* - \`$CI_PROJECT_NAME\` (\`$CI_COMMIT_SHORT_SHA\`)\",
          \"blocks\": [
            {\"type\":\"section\",\"text\":{\"type\":\"mrkdwn\",\"text\":\":white_check_mark: *Pipeline #$CI_PIPELINE_ID 성공*\n*Project:* $CI_PROJECT_NAME\n*Branch:* $CI_COMMIT_BRANCH\n*Commit:* \`$CI_COMMIT_SHORT_SHA\` - $CI_COMMIT_TITLE\n*Author:* $GITLAB_USER_NAME\n*Pipeline:* <$CI_PIPELINE_URL|링크>\"}}
          ]
        }"
  rules:
    - if: $CI_COMMIT_BRANCH == "main"
  ...
  script:
    ...
    - |
      curl -X POST "$SLACK_WEBHOOK_URL" \
        -H 'Content-type: application/json' \
        -d "{
          \"text\": \":x: *Pipeline 실패* - \`$CI_PROJECT_NAME\` (\`$CI_COMMIT_SHORT_SHA\`)\",
          \"blocks\": [
            {\"type\":\"section\",\"text\":{\"type\":\"mrkdwn\",\"text\":\":x: *Pipeline #$CI_PIPELINE_ID 실패*\n*Project:* $CI_PROJECT_NAME\n*Branch:* $CI_COMMIT_BRANCH\n*Commit:* \`$CI_COMMIT_SHORT_SHA\` - $CI_COMMIT_TITLE\n*Author:* $GITLAB_USER_NAME\n*Pipeline:* <$CI_PIPELINE_URL|링크>\n*Failed Job:* <$CI_JOB_URL|확인>\"}}
          ]
        }"
  rules:
    - if: $CI_COMMIT_BRANCH == "main"
...
1. gitlab.com → tutum-project/tutum-app → CI/CD → Pipelines → Run pipeline
   → 모든 스테이지(lint → test → scan → build → security → sign → deploy → notify) 통과 확인

2. gitlab.com → tutum-project/tutum-app → Packages & Registries → Container Registry
   → backend, frontend 이미지가 push되어 있는지 확인

3. SonarQube (http://192.168.56.30:9000) → tutum-backend / tutum-frontend
   → 코드 분석 결과 확인, Quality Gate 통과 여부

4. Slack #tutum-cicd 채널
   → Pipeline 성공/실패 알림이 도착하는지 확인

5. Jira (https://<domain>.atlassian.net/browse/TUTUM)
   → GitLab 커밋 메시지에 TUTUM-XXX 포함 시 이슈에 코멘트 자동 추가 확인
```
...
- [ ] SonarQube(monitoring VM:9000) 설치 + 프로젝트 2개(backend/frontend) + Quality Gate 설정
- [ ] GitLab Runner가 K8s 클러스터에서 Running 상태
- [ ] CI Variables 등록 완료 (SONAR_TOKEN, COSIGN_KEY, COSIGN_PASSWORD, DEPLOY_TOKEN, SLACK_WEBHOOK_URL)
...
- [ ] Jira 연동 완료 (gitlab.com ↔ Jira Cloud, Smart Commit 동작 확인)
- [ ] Slack 연동 완료 (Pipeline 성공/실패 알림 #tutum-cicd 채널 수신 확인)
- [ ] 테스트 파이프라인 전체 스테이지 통과 (lint → build → push → notify)
...
1. Alerting → Contact points → Add contact point
   - Name: slack-alerts
   - Integration: Slack
   - Webhook URL: (Slack #tutum-alerts 채널 Incoming Webhook URL)
   - Title: {{ template "default.title" . }}
   - Text Body: {{ template "default.message" . }}
   → Test → Save

2. Alerting → Notification policies → Default policy 수정
   - Default contact point: slack-alerts
   → 모든 알림이 Slack으로 전송됨

3. 테스트 알림 발송:
   → Alerting → Alert rules → 아무 규칙 선택 → "Test" 버튼
   → Slack #tutum-alerts 채널에 알림 도착 확인
```

**알림 채널 구분:**
| Slack 채널 | 용도 | 발송 주체 |
|-----------|------|----------|
| #tutum-cicd | CI/CD Pipeline 성공/실패, 배포 알림 | GitLab CI (.gitlab-ci.yml notify 스테이지) |
| #tutum-alerts | 인프라/앱 장애 알림 (CPU, 메모리, Pod Down 등) | Grafana Alerting |
...
- [ ] 알림 규칙 5개 이상 설정 완료
- [ ] Grafana → Slack 알림 연동 (#tutum-alerts 채널 수신 확인)
- [ ] 테스트 알림 발송 확인 (CPU/메모리 threshold alert → Slack 도착)
...
4. Slack 알림 (#tutum-cicd)
   → Pipeline 성공/실패 알림 + Jira 이슈 자동 코멘트
...
8. 장애 발생 시 Grafana 알림 (#tutum-alerts)
   → CPU/메모리 threshold 초과, Pod 비정상 등 → Slack 알림
```
...
| **SonarQube** | monitoring VM | 192.168.56.30 | 9000 | 브라우저, CI Runner |
...
> - 브라우저 → SonarQube: `http://192.168.56.30:9000` (Host-Only 네트워크)
> - K8s Runner → SonarQube: `http://192.168.56.30:9000` (Host-Only 네트워크)
> - K8s Runner → GitLab/Registry: 인터넷 경유 (HTTPS)
...
> - Slack/Jira: 인터넷 경유 (HTTPS) — CI Runner, Grafana에서 발송
...
2. **CI/CD 전용 VM(192.168.56.40) 사용하지 않음** — gitlab.com (SaaS) + SonarQube (Monitoring VM) + GitLab Container Registry 사용
3. **SonarQube는 Monitoring VM(192.168.56.30:9000)에 설치** — Phase 2에서 Docker Compose로 설치, Phase 3 LGTM과 같은 VM에서 운영
4. **GitLab Runner는 K8s 클러스터 내 Helm chart로 설치** — 별도 VM 불필요, CI/CD 실행 시간 무제한
5. **imagePullSecrets 필수** — K8s가 GitLab Container Registry(registry.gitlab.com)에서 이미지 pull하려면 Deploy Token + K8s Secret 필요
6. **K8s 노드에서 인터넷 접근 필수** — gitlab.com, registry.gitlab.com 접근 (NAT 어댑터로 이미 가능)
7. **Slack/Jira 연동** — CI/CD Pipeline 알림은 #tutum-cicd, LGTM 모니터링 알림은 #tutum-alerts 채널로 분리
8. **MariaDB는 외부 서버** — K8s에 포함하지 않음, 학원 온프레미스 서버에서 IP:Port 접속
9. **MongoDB Atlas → K8s StatefulSet 전환**은 이 계획 범위 밖 (데이터 레이어 마이그레이션은 별도 계획)
10. **KEDA, Karpenter는 이 계획 범위 밖** — K8s 클러스터 기본 뼈대 + CI/CD + LGTM에 집중
11. **Istio는 기본 설치만** — Gateway + mTLS까지만 설정, Canary/Blue-Green 배포 전략은 별도 계획
12. **K8s 버전**: 설치 시점 기준 최신 안정 버전 확인 (현재 기준 v1.29)
