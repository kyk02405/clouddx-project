...
# GitLab Container Registry (registry.gitlab.com) + gitlab.com + SonarQube(192.168.56.40:9000) 접근
...
> **중요**: CI/CD는 **gitlab.com (SaaS)** + **SonarQube(192.168.56.40:9000)** + **GitLab Container Registry**를 사용합니다.
> GitLab Runner는 K8s 클러스터 내에 Helm chart로 설치하고, SonarQube는 CI/CD VM(192.168.56.40)에서 운영합니다.
...
```
1. 브라우저에서 http://192.168.56.40:9000 접속
...
- SonarQube/Harbor는 현재 CI/CD VM 단일 운영(예산/운영 부담 고려).
...
git remote add gitlab https://gitlab.com/tutum-group/tutum-project/tutum-app.git
git push -u gitlab --all

# k8s-manifests 레포 생성 + push (별도 디렉토리에서)
mkdir k8s-manifests && cd k8s-manifests
git init
git remote add origin https://gitlab.com/tutum-group/tutum-project/k8s-manifests.git
# (2-7에서 디렉토리 구조 작성 후 push)
```

> gitlab.com 접속: `https://gitlab.com/tutum-group/tutum-project`
> Container Registry: `registry.gitlab.com/tutum-group/tutum-project/tutum-app`
...
gitlab.com → tutum-group/tutum-project/tutum-app → Settings → CI/CD → Variables:

```
SONAR_TOKEN        = (SonarQube에서 발급한 토큰)
SONAR_HOST_URL     = http://192.168.56.40:9000
...
argocd repo add https://gitlab.com/tutum-group/tutum-project/k8s-manifests.git \
...
> GitLab URL: `https://gitlab.com/tutum-group/tutum-project/k8s-manifests.git` (인터넷 경유)
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: frontend
          image: registry.gitlab.com/tutum-group/tutum-project/tutum-app/frontend:latest
          ports:
            - containerPort: 3000
          resources:
            requests:
...
# gitlab.com → tutum-group/tutum-project/tutum-app → Settings → Repository → Deploy tokens
...
spec:
  project: default
  source:
    repoURL: https://gitlab.com/tutum-group/tutum-project/k8s-manifests.git
    targetRevision: main
...
spec:
  ...
  rules:
    - name: verify-cosign-signature
      ...
      verifyImages:
        - imageReferences:
            - "registry.gitlab.com/tutum-group/tutum-project/*"
          attestors:
            - entries:
                - keys:
                    publicKeys: |-
...
deploy:staging:
  ...
  script:
    - git clone https://$DEPLOY_TOKEN@gitlab.com/tutum-group/tutum-project/k8s-manifests.git
...
**gitlab.com 웹 UI에서:**
```
1. gitlab.com → tutum-group/tutum-project/tutum-app → CI/CD → Pipelines → Run pipeline
   → 모든 스테이지(lint → test → scan → build → security → sign → deploy) 통과 확인

2. gitlab.com → tutum-group/tutum-project/tutum-app → Packages & Registries → Container Registry
   → backend, frontend 이미지가 push되어 있는지 확인

3. SonarQube (http://192.168.56.40:9000) → 프로젝트 결과 확인
...
crictl pull registry.gitlab.com/tutum-group/tutum-project/tutum-app/backend:latest 2>&1 || echo "이미지 없으면 정상 에러"
```
...
| **SonarQube** | CI/CD VM (Self-hosted) | 192.168.56.40 | 9000 (HTTP) | 브라우저, CI Runner |
...
> **CI/CD 도구 접근 경로 정리:**
> - 브라우저 → GitLab: `https://gitlab.com/tutum-group/tutum-project` (인터넷)
> - 브라우저 → SonarQube: `http://192.168.56.40:9000`
> - K8s Runner → GitLab/SonarQube/Registry: 인터넷 경유 (HTTPS)
> - ArgoCD(K8s) → GitLab: `https://gitlab.com/tutum-group/tutum-project/k8s-manifests.git` (인터넷 경유)
...
2. **CI/CD 전용 VM(192.168.56.40)에서 SonarQube 운영** — GitLab Runner(K8s)와 연동
...
5. **K8s 노드에서 인터넷 접근 필수** — gitlab.com, registry.gitlab.com, 192.168.56.40:9000 접근
