...
class KISClient:
    ...
    async def get_current_price(self, code: str, market: str = "KR"):
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                token_retry_codes = {"EGW00121", "EGW00123"}
                for attempt in range(2):
                    ...
                    for ex in exch_list:
                        ...
                        if data.get("msg_cd") in token_retry_codes and attempt == 0:
                    ...
                    if data and data.get("msg_cd") in token_retry_codes and attempt == 0:
                    ...
                    msg1 = (data or {}).get("msg1") or "KIS price unavailable"
                    raise ValueError(f"{msg_cd or 'KIS_NO_PRICE'}: {msg1}")
            except Exception as e:
